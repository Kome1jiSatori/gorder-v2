...
import (
	...
	"github.com/Kome1jiSatori/gorder-v2/common/discovery/consul"
...
func GetServiceAddr(ctx context.Context, serviceName string) (addr string, err error) {
	registry, err := consul.New(viper.GetString("consul.addr"))
	if err != nil {
		return "", err
	}
	...
	logrus.Infof("Discovered %s instance of %s, addrs=%v", len(addrs), serviceName, addrs)
